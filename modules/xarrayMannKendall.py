"""
Filename:    xarrayMannKendall.py
Author:      josuemtzmo, https://github.com/josuemtzmo
Description: Functions used for running Mann-Kendall test on xarray dataset (https://github.com/josuemtzmo/xarrayMannKendall)
"""
...
# Import my modules (modfied for local use)
...
class Mann_Kendall_test(object):
    ...
    def compute(self,save=False,path=None):
        """
        Wrapper to compute trends and returns a xr.Dataset containing 
        ...
        if len(self.ordered_dims) == 2:
            ...
            ds = xr.Dataset({'trend': (['y', 'x'], MK_output[:,:,0]),
                        ...
                        coords={'x': (['x'], self.DataArray.x.values),
                                'y': (['y'], self.DataArray.y.values)})
        if path != None:
            ds.to_netcdf(path)
